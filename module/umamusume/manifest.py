...
from module.umamusume.scenario.registry import REGISTRY
...
def build_cultivate_script_dict():
    result = dict(base_script_dict)
    for scenario_type, scenario_cls in REGISTRY.items():
        scenario = scenario_cls()
        handlers = scenario.get_ui_handlers()
        result.update(handlers)
    return result

script_dicts: Dict[UmamusumeTaskType, dict] = {
    UmamusumeTaskType.UMAMUSUME_TASK_TYPE_CULTIVATE: build_cultivate_script_dict()
}
